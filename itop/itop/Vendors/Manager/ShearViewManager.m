...
-(void)setupShearViewWithshearType:(ShearType)shearType;{
    ...
    [self shearTypeWithShearType:shearType];
...
//    };
    ...
    [shearView setupShearItemWithshearTepy:shearType];
    [self setupPreDefinePlatforms];
    
    shearView.cancelBlock = ^(id obj){
        
        [weakSelf hiddenBgViewAndPickerView:nil];
    };
...
-(NSArray *)shearTypeWithShearType:(ShearType)shearType{
   
    
    if (shearType == ShearTypeProduct) {
        
        _shearTypeArray = @[@(UMSocialPlatformType_WechatTimeLine),@(UMSocialPlatformType_WechatSession),@(UMSocialPlatformType_Qzone),@(UMSocialPlatformType_QQ),@(UMSocialPlatformType_Sina)];
    } else {
        _shearTypeArray =  @[@(UMSocialPlatformType_WechatTimeLine),@(UMSocialPlatformType_WechatSession),@(UMSocialPlatformType_QQ),@(UMSocialPlatformType_Sina)];
    }
    
    return self.shearTypeArray;
...
- (void)addShearViewToView:(UIView*)view
    ...
    shearView.selectShearItme = ^(NSInteger tag){
        ...
        if (tag < self.shearTypeArray.count && [weakSelf opinionInstallAppWithTag:tag]) {
            
            platform = [self.shearTypeArray[tag] integerValue];
            completion(tag);
...
-(void)hiddenBgViewAndPickerView:(UITapGestureRecognizer *)sender{
    
    CGPoint point = [sender locationInView:[[UIManager sharedUIManager]topViewController].view];
    NSLog(@"handleSingleTap!pointx:%f,y:%f",point.x,point.y);
    
    NSLog(@"%f",ScreenHeigh);
    
    CGFloat shearHeight = _shearType == ShearTypeProduct ? 251 : 160;
    //添加滚动动画
    if (point.y < ScreenHeigh - shearHeight) {
...
-(void)editoeViewWithAnimation:(BOOL)animation{
    ...
    [UIView animateWithDuration:0.2 animations:^{
        ...
        if (animation) {
            shearView.frame = CGRectMake(0, ScreenHeigh-shearHeight*KadapterH, ScreenWidth, 245*KadapterH);
        }else {
            shearView.frame = CGRectMake(0, ScreenHeigh, ScreenWidth, 300*KadapterH);
        }
...
//        }
    ...
    [self hiddenBgViewAndPickerView:nil];
...
-(BOOL)opinionInstallAppWithTag:(NSInteger)tag{
    ...
    }else if ([tagStr isEqualToString:@"QQ好友"] || [tagStr isEqualToString:@"QQ空间"]){
    ...
    }else if ([tagStr isEqualToString:@"新浪微博"] ){
