...
-(instancetype)initWithFrame:(CGRect)frame withDataArray:(NSArray *)dataArray withFont:(CGFloat)font {
    ...
    if (self) {
        ...
        self.textSelectedColor = RGB(75, 139, 217);
        self.lineColor = RGB(75, 139, 217);
        
        [self addSubSegmentView];
//        [self segmentationLineWithFrame:frame.size.height-1];
    }
    return self;
}

-(void)segmentationLineWithFrame:(CGFloat)h{
    
    UIView *segmentationLineView = [[UIView alloc]initWithFrame:CGRectMake(0, h, ScreenWidth, 1)];
    segmentationLineView.backgroundColor = RGB(234, 235, 237);
    [self addSubview:segmentationLineView];
}
...
//        }
    ...
    self.lineImageView = [[UIImageView alloc] initWithFrame:CGRectMake(0, self.frame.size.height-2.5, width, 2.5)];
...
-(void)tapAction:(id)sender{
    ...
    [UIView animateWithDuration:0.2 animations:^{
       self.lineImageView.frame = CGRectMake(button.frame.origin.x, self.frame.size.height-2.5, button.frame.size.width, 2.5);
    }];
...
-(void)selectIndex:(NSInteger)index
{
    for (UIButton *subButton in self.buttonsArray) {
        if (index != subButton.tag) {
            ...
            subButton.selected = YES;
            [UIView animateWithDuration:0.2 animations:^{
                self.lineImageView.frame = CGRectMake(subButton.frame.origin.x, self.frame.size.height-2.5, subButton.frame.size.width, 2.5);
            }];
        }
    }
}
