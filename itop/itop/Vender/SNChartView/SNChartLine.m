...
static const CGFloat kTopSpace = 15.f;//距离顶部y值
...
- (void)drawHorizontal {
    ...
    for (NSInteger i = 0; i <= kYEqualPaths; i++) {
        ...
        [path addLineToPoint:CGPointMake(chartLineStartX + (_xValues.count - 1) * chartLineTheXAxisSpan, chartLineTheYAxisSpan * i + kTopSpace)];
        [path closePath];
...
- (void)addYLabel {
    for (NSInteger i = 0; i <= kYEqualPaths; i++) {
        ...
        if (i == kYEqualPaths) {
            ...
            label.text = [NSString stringWithFormat:@"%ld%%",20 * (kYEqualPaths - i)];
        }
    }
}
//圆圈点击事件
- (void)circleButtonClick:(UIButton *)btn {
  ...
  NSString *bulldes = [NSString stringWithFormat:@"%.0f%%",[content floatValue]*100];
    CGSize size = [bulldes sizeWithAttributes:@{NSFontAttributeName:kBulldesFont}];
    if (size.width < 25.f) {
        size.width = 25.f;
    }
    [self addBulldesView];
    _bulldesImageView.frame = CGRectMake(x-(size.width)/2.0 + 3, y - 20.f, size.width + 5, 20);
    _bulldesLabel.frame = CGRectMake(0, 1, _bulldesImageView.frame.size.width, 10);
    _bulldesLabel.text = bulldes;
}
