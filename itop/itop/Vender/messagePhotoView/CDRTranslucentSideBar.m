...
//    self.sideBarWidth = self.view.frame.size.width - 200;
//    self.sideBarHeight = 0;
    self.animationDuration = 0.20f;
    self.sideBarWidth = self.view.frame.size.width;
...
- (void)initTranslucentView
{
    if (SYSTEM_VERSION_GREATER_THAN_OR_EQUAL_TO(@"7.0")) {
        ...
        CGRectMake(self.showFromRight ? self.view.bounds.size.width : -self.sideBarWidth, 64, self.sideBarWidth, 0);
...
- (void)layoutSubviews
{
    ...
    if (self.contentView != nil) {
        self.contentView.frame = CGRectMake(x, 64, self.sideBarWidth, self.parentViewController.view.bounds.size.height-64);
    }
}
...
- (void)showInViewController:(UIViewController *)controller animated:(BOOL)animated
{
    ...
    CGRect sideBarFrame = CGRectMake(0, 64, self.view.bounds.size.width,100 );
...
//    sideBarFrame.origin.x = parentWidth;
    void (^animations)() = ^{
        if (self.contentView != nil) {
            self.contentView.frame = sideBarFrame;
        }
...
- (void)addToParentViewController:(UIViewController *)parentViewController callingAppearanceMethods:(BOOL)callAppearanceMethods
{
    ...
    if (callAppearanceMethods) [self beginAppearanceTransition:YES animated:YES];
